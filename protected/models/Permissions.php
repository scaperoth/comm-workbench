...
class Permissions extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('permission_type', 'required'),
			array('permission_type', 'length', 'max'=>45),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('permission_id, permission_type', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'users' => array(self::MANY_MANY, 'User', 'has_permissions(permission_id, usr_id)'),
		);
	}
